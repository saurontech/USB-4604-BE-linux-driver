...
static int xr_usb_serial_tty_open(struct tty_struct *tty, struct file *filp)
{
	...
	retval = tty_port_open(&xr_usb_serial->port, tty, filp);
	if (retval == 0)
		adv_usb_serial_set_mode(xr_usb_serial);
	return retval;
}
						...
						struct ktermios *termios_old)
{
	...
	if(xr_usb_serial->mode == 232)	
		...
		adv_usb_serial_set_soft_flow_mode(xr_usb_serial,tty);/*set the serial soft flow mode*/
...
static int adv_proc_show(struct seq_file *m, void *v)
{
	struct xr_usb_serial *xr_usb_serial = m->private;

	seq_printf(m, "Minor is %d, Mode is %d\n", xr_usb_serial->minor, xr_usb_serial->mode);
		...
		size_t count, loff_t *pos)
{
	...
	if(!strncmp("232", cmd, len - 1))
		xr_usb_serial->mode = 232;
	else if(!strncmp("422", cmd, len - 1))
		xr_usb_serial->mode = 422;
	else if(!strncmp("485", cmd, len - 1))
		xr_usb_serial->mode = 485;
	else
		return -EINVAL;
...
skip_countries:
	...
	if(xr_usb_serial->DeviceProduct == 0xB704){
		sprintf(filename, "driver/adv-usb-serial%d", minor);
		proc_file_entry = proc_create_data(filename, 0666, NULL, &adv_proc_fops, xr_usb_serial);
		if(proc_file_entry == NULL){
			printk("Advantech USB to serial driver proc register failed!\n");
			return -ENOMEM;
		}else
			printk("Advantech USB to serial driver proc created\n");
	}
...
static void xr_usb_serial_disconnect(struct usb_interface *intf)
{
	...
	if(xr_usb_serial->DeviceProduct == 0xB704){
		sprintf(filename, "driver/adv-usb-serial%d", xr_usb_serial->minor);
		remove_proc_entry(filename, NULL);
		printk("Advantech USB to serial driver proc removed\n");
	}
...
static const struct usb_device_id xr_usb_serial_ids[] = {
    ...
    { USB_DEVICE(0x1809, 0xB604)},
    { USB_DEVICE(0x1809, 0xB704)},
	{ }
};
...
static int __init xr_usb_serial_init(void)
{
	...
	xr_usb_serial_tty_driver->name = "ttyADVUSB",
